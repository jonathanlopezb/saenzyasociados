...
import { ShieldCheck, Activity, FileText, Download, Calendar, CheckCircle2, Clock, Wrench, AlertTriangle, FileSpreadsheet, Image as ImageIcon } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-blue-700 to-[#0077FF] border border-[#00D2FF]/50 text-white text-xs font-bold uppercase tracking-wider mb-4 shadow-xl">
            ...
            <span>Portal Exclusivo para Empresas Contratantes</span>
          </div>

          <h2 className="text-3xl sm:text-5xl font-extrabold text-white tracking-tight mb-4">
            Avances de Proyectos & <br className="hidden sm:inline" />
            <span className="bg-gradient-to-r from-[#00D2FF] via-[#3393FF] to-white bg-clip-text text-transparent">
              Reportes Diarios B2B en Vivo
            </span>
          </h2>

          <p className="text-sm sm:text-base text-slate-300">
            Transparencia total para interventores y gerentes de operaciones. Consulte diariamente el historial de trabajos ejecutados, registros fotográficos y actas técnicas.
          </p>
        </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-lg font-bold text-white">{CLIENT_PORTAL_MOCK.clientName}</h3>
              ...
              <p className="text-xs text-slate-400">NIT: {CLIENT_PORTAL_MOCK.nit} | {CLIENT_PORTAL_MOCK.activeContract}</p>
            </div>
          </div>

          <div className="flex items-center gap-3">
            <button className="inline-flex items-center gap-2 bg-[#0B1F3A] hover:bg-[#0077FF]/20 border border-[#0077FF]/40 text-[#00D2FF] font-bold text-xs px-4 py-2.5 rounded-xl transition-all">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-6 border-b border-slate-800 pb-3">
          ...
          >
            ...
            Inventario de Equipos ({CLIENT_PORTAL_MOCK.equipmentList.length})
          </button>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pt-2 text-xs text-slate-400">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="flex items-center gap-1 text-[#00D2FF]">
                      ...
                      {report.photosCount} Fotos de Avance
                    </span>
                    <button
                      onClick={() => alert(`Descargando copia digital firmada del reporte ${report.pdfLog}`)}
                      className="inline-flex items-center gap-1 text-xs font-bold text-white bg-[#0077FF] hover:bg-[#3393FF] px-3 py-1.5 rounded-lg transition-all"
                    >
        ...
        {activeTab === 'equipment' && (
          <div className="glass-panel rounded-2xl overflow-hidden border border-[#0077FF]/30 animate-in fade-in duration-300">
          ...
          <div className="space-y-3 animate-in fade-in duration-300">
            {CLIENT_PORTAL_MOCK.invoices.map((inv) => (
              <div key={inv.id} className="glass-panel p-4 rounded-xl border border-slate-800 flex items-center justify-between text-xs">
                ...
                <button 
                  onClick={() => alert(`Simulación de descarga de factura ${inv.id}`)}
                  className="flex items-center gap-1 text-xs font-bold text-[#00D2FF] bg-[#0077FF]/20 border border-[#0077FF]/40 px-3 py-2 rounded-xl"
                >
